	...
	if #units > 0 then
		Timers:CreateTimer(explode_delay, function()
			if target:IsAlive() then
				--Функции ниже помогают воспроизвести эффект взрыва
				local particle_explosion = "particles/units/heroes/hero_techies/techies_land_mine_explode.vpcf"
				local particle_explosion_fx = ParticleManager:CreateParticle(particle_explosion, PATTACH_WORLDORIGIN, target)
				ParticleManager:SetParticleControl(particle_explosion_fx, 0, target:GetAbsOrigin())
				ParticleManager:SetParticleControl(particle_explosion_fx, 1, target:GetAbsOrigin())
				ParticleManager:SetParticleControl(particle_explosion_fx, 2, Vector(damage_radius, 1, 1))
				ParticleManager:ReleaseParticleIndex(particle_explosion_fx)
				
				-- Explode sound
				EmitSoundOnLocationWithCaster( target:GetAbsOrigin(), "Hero_Techies.LandMine.Detonate", target )
